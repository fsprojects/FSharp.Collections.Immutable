...
module Queue =
    ...
    let clear queue : IQueue<_> =
        check queue
        queue.Clear ()

    let enqueue item queue : IQueue<_> =
        check queue
        queue.Enqueue item
    ...
    let filter predicate queue =
        let rec loop queue result =
            ...
            | Cons (head, tail) ->
                loop tail
                <| if predicate head then enqueue head result else result
            | Nil -> result
    ...
    let find predicate queue =
        check queue
        Seq.find predicate queue

    let tryFind predicate queue =
        check queue
        Seq.tryFind predicate queue

    let findIndex predicate queue =
        check queue
        Seq.findIndex predicate queue

    let tryFindIndex predicate queue =
        check queue
        Seq.tryFindIndex predicate queue

    let pick chooser queue =
        check queue
        Seq.pick chooser queue

    let tryPick chooser queue =
        check queue
        Seq.tryPick chooser queue

    let iter action queue =
        check queue
        Seq.iter action queue

    let iteri action queue =
        check queue
        Seq.iteri action queue
    ...
    let fold folder state queue =
        check queue
        Seq.fold folder state queue
    ...
    let exists predicate queue =
        check queue
        Seq.exists predicate queue

    let reduce reduction queue =
        check queue
        Seq.reduce reduction queue

    let inline sum queue =
        check queue
        Seq.sum queue

    let inline sumBy projection queue =
        check queue
        Seq.sumBy projection queue

    let inline average queue =
        check queue
        Seq.average queue

    let inline averageBy projection queue =
        check queue
