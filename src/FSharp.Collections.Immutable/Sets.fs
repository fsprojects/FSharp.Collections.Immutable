...
module HashSet =
    ...
    let inline toArray (set : HashSet<_>) =
        check set
        Seq.toArray set
    ...
    let inline toBuilder set : HashSetBuilder<_> =
        check set
        set.ToBuilder ()

    let inline keyComparer set =
        check set
        set.KeyComparer

    let inline length set =
        check set
        set.Count

    let inline isEmpty set =
        check set
        set.IsEmpty

    let inline contains value set =
        check set
        set.Contains value

    let inline exists predicate map =
        check map
        map |> Seq.exists predicate

    let inline isSubset (set1 : HashSet<_>) set2 =
        check set1
        set1.IsSubsetOf set2

    let inline isProperSubset (set1 : HashSet<_>) set2 =
        check set1
        set1.IsProperSubsetOf set2

    let inline isSuperset (set1 : HashSet<_>) set2 =
        check set1
        set1.IsSupersetOf set2

    let inline isProperSuperset (set1 : HashSet<_>) set2 =
        check set1
        set1.IsProperSupersetOf set2

    let inline add value set : HashSet<_> =
        check set
        set.Add (value)

    let inline union set values : HashSet<_> =
        check set
        values |> set.Union

    let inline unionMany (sets : HashSet<_> seq) = Seq.reduce union sets

    let inline intersect (set1 : HashSet<_>) set2 =
        check set1
        set1.Intersect set2

    let inline intersectMany (sets : HashSet<_> seq) = Seq.reduce intersect sets

    let inline remove value set : HashSet<_> =
        check set
        set.Remove value

    let inline difference values set : HashSet<_> =
        check set
        values |> set.Except

    let inline clear set : HashSet<_> =
        check set
        set.Clear ()
    ...
    let inline pick chooser set =
        check set
        set |> Seq.pick chooser

    let inline tryPick chooser set =
        check set
        set |> Seq.tryPick chooser
    ...
    let inline iter action (set : HashSet<_>) =
        check set
        set |> Seq.iter action
...
module SortedSet =
    ...
    let inline toArray (set : SortedSet<_>) =
        check set
        Seq.toArray set
    ...
    let inline toBuilder set : SortedSetBuilder<_> =
        check set
        set.ToBuilder ()
    ...
    let inline isSubset (set1 : SortedSet<_>) set2 =
        check set1
        set1.IsSubsetOf set2

    let inline isProperSubset (set1 : SortedSet<_>) set2 =
        check set1
        set1.IsProperSubsetOf set2

    let inline isSuperset (set1 : SortedSet<_>) set2 =
        check set1
        set1.IsSupersetOf set2

    let inline isProperSuperset (set1 : SortedSet<_>) set2 =
        check set1
        set1.IsProperSupersetOf set2

    let inline add value set : SortedSet<_> =
        check set
        set.Add (value)

    let inline union set values : SortedSet<_> =
        check set
        values |> set.Union
    ...
    let inline remove value set : SortedSet<_> =
        check set
        set.Remove value

    let inline difference values set : SortedSet<_> =
        check set
        values |> set.Except

    let inline clear set : SortedSet<_> =
        check set
        set.Clear ()
    ...
    let inline iter action (set : SortedSet<_>) =
        check set
        set |> Seq.iter action
