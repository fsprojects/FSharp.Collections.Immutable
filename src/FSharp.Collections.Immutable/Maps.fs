...
type HashMapBuilder<'Key, 'Value when 'Key : not null> = HashMap<'Key, 'Value>.Builder
...
module HashMap =
    ...
    let inline empty<'Key, 'Value when 'Key : not null> = HashMapFactory.Create<'Key, 'Value> ()
    ...
    let inline toArray (map : HashMap<_, _>) =
        check map
        Seq.toArray map
    ...
    let inline toBuilder map : HashMapBuilder<_, _> =
        check map
        map.ToBuilder ()

    let inline ofKeyComparer<'Key, 'Value when 'Key : not null> comparer = HashMapFactory.Create<'Key, 'Value> (comparer)
    let inline ofComparers<'Key, 'Value when 'Key : not null> keyComparer valueComparer =
        HashMapFactory.Create<'Key, 'Value> (keyComparer, valueComparer)


    let inline isEmpty map =
        check map
        map.IsEmpty

    let inline length map =
        check map
        map.Count

    let inline keyComparer map =
        check map
        map.KeyComparer

    let inline valueComparer map =
        check map
        map.ValueComparer

    let inline containsKey key map =
        check map
        map.ContainsKey key

    let inline find key map =
        check map
        map.[key]
    ...
    let inline pick chooser map =
        check map
        map |> Seq.pick (fun kvp -> chooser kvp.Key kvp.Value)

    let inline tryPick chooser map =
        check map
        map |> Seq.tryPick (fun kvp -> chooser kvp.Key kvp.Value)
    ...
    let inline iter action (map : HashMap<_, _>) =
        check map
        map |> Seq.iter (fun kvp -> action kvp.Key kvp.Value)

    let inline exists predicate map =
        check map
        map |> Seq.exists (fun kvp -> predicate kvp.Key kvp.Value)

    let inline add key value map : HashMap<_, _> =
        check map
        map.Add (key, value)
    ...
    let inline remove key map : HashMap<_, _> =
        check map
        map.Remove key

    let inline except keys map : HashMap<_, _> =
        check map
        map.RemoveRange keys

    let inline clear map : HashMap<_, _> =
        check map
        map.Clear ()

    let inline filter predicate map =
        map
        |> Seq.filter (fun (kvp : KeyValuePair<_, _>) -> predicate kvp.Key kvp.Value)

    let inline forall predicate map =
        map
        |> Seq.forall (fun (kvp : KeyValuePair<_, _>) -> predicate kvp.Key kvp.Value)

    let inline map mapping map' =
        map'
        |> Seq.map (fun (kvp : KeyValuePair<_, _>) -> mapping kvp.Key kvp.Value)
        |> ofSeq

    let inline where predicate map =
        map
        |> Seq.where (fun (kvp : KeyValuePair<_, _>) -> predicate kvp.Key kvp.Value)
        |> empty.AddRange


type SortedMap<'Key, 'Value when 'Key : not null> = System.Collections.Immutable.ImmutableSortedDictionary<'Key, 'Value>

type SortedMapBuilder<'Key, 'Value when 'Key : not null> = SortedMap<'Key, 'Value>.Builder
...
module SortedMap =
    ...
    let inline empty<'Key, 'Value when 'Key : not null> = SortedMapFactory.Create<'Key, 'Value> ()
    ...
    let inline toArray (map : SortedMap<_, _>) =
        check map
        Seq.toArray map
    ...
    let inline toBuilder map : SortedMapBuilder<_, _> =
        check map
        map.ToBuilder ()

    let inline ofKeyComparer<'Key, 'Value when 'Key : not null> comparer = SortedMapFactory.Create<'Key, 'Value> (comparer)
    let inline ofComparers<'Key, 'Value when 'Key : not null> keyComparer valueComparer =
        SortedMapFactory.Create<'Key, 'Value> (keyComparer, valueComparer)
    ...
    let inline iter action (map : SortedMap<_, _>) =
        check map
        map |> Seq.iter (fun kvp -> action kvp.Key kvp.Value)
    ...
    let inline add key value map : SortedMap<_, _> =
        check map
        map.Add (key, value)
    ...
    let inline remove key map : SortedMap<_, _> =
        check map
        map.Remove key

    let inline except keys map : SortedMap<_, _> =
        check map
        map.RemoveRange keys

    let inline clear map : SortedMap<_, _> =
        check map
        map.Clear ()
    ...
    let inline tryFindKey predicate map =
        check map
        map |> Seq.tryFind (fun kvp -> predicate kvp.Key kvp.Value)
