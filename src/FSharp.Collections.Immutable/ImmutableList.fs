...
module ImmutableList =
    ...
    let inline ofArray (source : _ array) =
        checkNotNull (nameof source) source
        ImmutableList.CreateRange source
    ...
    let inline toArray (list : ImmutableList<_>) =
        check list
        Seq.toArray list
    ...
    let toBuilder (list : ImmutableList<_>) =
        check list
        list.ToBuilder ()
    ...
    let replace oldValue value list = replaceWith HashIdentity.Structural oldValue value list
    ...
    let init count initializer =
        if count < 0 then
            ...
            with exn ->
                raise exn // get the right stack trace
    ...
    let forall2 predicate (list1 : IImmutableList<_>) (list2 : IImmutableList<_>) =
        checkNotNull (nameof list1) list1
        checkNotNull (nameof list2) list2
